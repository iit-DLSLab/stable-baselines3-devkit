...
from algos.policies.modules.tcn import TCN


class TCNPolicy(BasePolicy):
    ...
    def __init__(
        ...
        dim_model: int = 512,
        tcn_channels: list = [256, 256, 512, 512],
        kernel_size: int = 3,
        dropout: float = 0.1,
        ...
        **kwargs: Any,  # Note: we garbage-collect additional parameters, so double check the implementation details when using a configuration
        ...
        # TCN backbone network
        self.tcn_backbone = TCN(
    ...
    def compute_loss(self, obs, actions):
        ...
        tcn_features = self.tcn_backbone(images, state)  # (bs, time_seq, dim_model)
        joint_deltas = self.action_net(tcn_features)  # (bs, time_seq, n_joints)
    ...
    def predict(self, obs: Dict[str, Tensor]) -> Tuple[Tensor, Union[Tuple[Tensor, Tensor], Tuple[None, None]]]:
        ...
        with torch.no_grad():
            ...
            # Forward through TCN
            tcn_features = self.tcn_backbone(images, state)
            joint_deltas = self.action_net(tcn_features)
    ...
    def predict_seq(self, obs: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        bs, time_seq, num_cameras, c, h, w = images_seq.shape

        tcn_features = self.tcn_backbone(images_seq, state_seq)
        joint_deltas = self.action_net(tcn_features)
