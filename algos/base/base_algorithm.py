"""Abstract base classes for SB3 algorithms."""
...
from algos.policies.modules.nn_utils import (
...
class BaseAlgorithm(ABC):
    """
    The base algorithm's class.

    :param policy: The policy model to use (MlpPolicy, CnnPolicy, ...)
    :param env: The environment to learn from
                (if registered in Gym, can be str. Can be None for loading trained models)
    :param lr_value: learning rate for the optimizer,
    ...
    :param stats_window_size: Window size for the rollout logging, specifying the number of environments to average
    ...
    def __init__(
        ...
        self.lr_linear_end_value = lr_linear_end_value
        self.lr_linear_end_fraction = lr_linear_end_fraction
        ...
        if env is not None:
            ...
            if policy in ["CnnPolicy"] and isinstance(self.observation_space, spaces.Dict):
                raise ValueError(
    ...
    def _setup_lr_schedule(self) -> None:
        ...
        def _set_lr(val):
            if self.lr_scheduler == "cosine":
                ...
                #     progress_remaining if (progress := 1.0 - progress_remaining) < self.lr_warmup_fraction else
                #     0.+(self.lr_value-0.) * (0.5*(1+math.cos(math.pi * (progress - self.lr_warmup_fraction) / (1-self.lr_warmup_fraction)))))
                return get_schedule_fn(get_cosine_schedule_with_warmup(val, self.lr_warmup_fraction))
            if self.lr_scheduler == "linear":
                return get_schedule_fn(get_linear_schedule(val, self.lr_linear_end_value, self.lr_linear_end_fraction))
            if self.lr_scheduler == "adaptive":
                return get_schedule_fn(get_constant_schedule(val))  # passthrough, it's adapted in the policy code
            if self.lr_scheduler == "constant" or self.lr_scheduler is None:
                return get_schedule_fn(get_constant_schedule(val))
            raise Exception("lr_scheduler must be 'linear', 'cosine', 'adaptive', or 'constant'")

        self.lr_schedule = (
            _set_lr(self.lr_value) if isinstance(self.lr_value, (int, float)) else [_set_lr(v) for v in self.lr_value]
        )
    ...
    def _update_learning_rate(self, optimizers: Union[List[th.optim.Optimizer], th.optim.Optimizer]) -> None:
        ...
        assert self.lr_schedule is not None, "`lr_schedule` is None. The schedule should be initialized calling `_setup_lr_schedule()` ideally at setup time of the algorithm class, in `_setup_model()`."

        self.lr_value = (
            self.lr_schedule(self._current_progress_remaining, self.lr_value)
            if isinstance(self.lr_value, (int, float))
            else [lr_schedule(self._current_progress_remaining, val) for lr_schedule, val in zip(self.lr_schedule, self.lr_value)]
        )
        self.logger.record("train/learning_rate", self.lr_value)

        if not isinstance(optimizers, list):
            optimizers = [optimizers]
        for optimizer in optimizers:
            update_learning_rate(optimizer, self.lr_value)
    ...
    def _setup_learn(
        ...
        if self.env is not None and (reset_num_timesteps or self._last_obs is None):
    ...
    def set_random_seed(self, seed: Optional[int] = None) -> None:
        ...
        th.use_deterministic_algorithms(False, warn_only=False)
    ...
    def load(  # noqa: C901
        ...
        overwrite_policy_arguments: bool = True,
        ...
        **kwargs,
        ...
        :param overwrite_policy_arguments: Whether to overwrite the stored policy arguments when loading the model
        ...
        :return: a new model instance with loaded parameters
        """
        ...
        data, params, pytorch_variables = load_from_zip_file(
            path if path else path[: -len(".zip")],
            device="cpu",
        ...
        if "policy_kwargs" in kwargs and "policy_kwargs" in data and kwargs["policy_kwargs"] != data["policy_kwargs"]:
            print("\n" + ("=" * 20))
            print("!! Warning !!")
            print("Input:\t", kwargs["policy_kwargs"])
            print("Checkpoint:\t", data["policy_kwargs"])
            if overwrite_policy_arguments:
                print("Overwriting stored policy kwargs with specified ones.")
            else:
                print(
                    "Policy kwargs provided not matching loaded ones are being ignored. If this behaviour is not desired, please set `overwrite_policy_arguments=True` in `load()` method."
                )
                kwargs["policy_kwargs"] = data["policy_kwargs"]
            print(("=" * 20) + "\n")

            # if not ignore_arguments_diff:
            #     raise ValueError(
            #         f"The specified policy kwargs do not equal the stored policy kwargs."
            #         f"\nStored kwargs: {data['policy_kwargs']}.\nSpecified kwargs: {kwargs['policy_kwargs']}"
            #     )

        if "observation_space" not in data or "action_space" not in data:
            print(
                "The observation_space and action_space are nor present in the checkpoint, therefore the consistency with the environment spaces cannot be verified."
            )
        else:
        ...
        if env is not None:
            ...
            env = cls._wrap_env(env, data.get("verbose", 1))
            # Check if given env is valid
            # check_for_correct_spaces(env, data["observation_space"], data["action_space"])
            # Discard `_last_obs`, this will force the env to reset before training
            # See issue https://github.com/DLR-RM/stable-baselines3/issues/597
            if force_reset and data is not None:
                data["ep_info_buffer"] = deque(maxlen=data["_stats_window_size"])
                data["_last_episode_starts"] = None
                data["_last_obs"] = None
            # `n_envs` must be updated. See issue https://github.com/DLR-RM/stable-baselines3/issues/1018
            if data is not None:
                data["n_envs"] = env.num_envs
        else:
            # Use stored env, if one exists (probably for prediction). If not, continue as is
            env = data.get("env")
        ...
        try:
            ...
            if model.preprocessor.overwrite_stats:
    ...
    def get_parameters(self) -> Dict[str, Dict]:
        ...
        for name in state_dicts_names:
            if name == "policy":
                attr = Accelerator.unwrap_model(None, self.policy)
            else:
                attr = recursive_getattr(self, name)
            # Retrieve state dict
