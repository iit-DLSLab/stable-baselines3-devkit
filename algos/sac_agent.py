...
from algos.policies.sac_policy import Actor, SACPolicy
...
class SAC(OffPolicyAlgorithm, InferenceInterface):
    """
    ...
    def __init__(
        ...
        :param gradient_steps: How many gradient steps to do after each rollout (see ``train_freq``).
            ...
            during the rollout. Defaults to 1.
        ...
        :param target_entropy: Target entropy when learning ``ent_coef`` (``ent_coef = 'auto'``).
            Set to 'auto' to use -dim(action_space) as target entropy. Defaults to "auto".
    ...
    def train(self, gradient_steps: int) -> None:
        ...
        for rollout_data in self.replay_buffer.get(self.batch_size):
            ...
            if ent_loss is not None and self.ent_coef_optimizer is not None:
            ...
            with torch.no_grad():
                ...
                discounts = rollout_data.discounts if hasattr(rollout_data, "discounts") else self.gamma
                # Select action according to policy
                next_actions, next_log_prob = self.actor.action_log_prob(rollout_data.next_observations)
                # Compute the next Q values: min over all critics targets
                next_q_values = torch.cat(self.critic_target(rollout_data.next_observations, next_actions), dim=1)
                next_q_values, _ = torch.min(next_q_values, dim=1)
                # add entropy term
                next_q_values = next_q_values - ent_coef * next_log_prob
                # td error + entropy term
                target_q_values = rollout_data.rewards + (1 - rollout_data.dones) * discounts * next_q_values
            ...
            critic_loss = 0.5 * sum(F.mse_loss(current_q.squeeze(-1), target_q_values) for current_q in current_q_values)
            ...
            # Compute actor loss: Alternative: actor_loss = torch.mean(log_prob - qf1_pi) Min over all critic networks
            q_values_pi = torch.cat(self.critic(rollout_data.observations, actions_pi), dim=1)
            min_qf_pi, _ = torch.min(q_values_pi, dim=1)
